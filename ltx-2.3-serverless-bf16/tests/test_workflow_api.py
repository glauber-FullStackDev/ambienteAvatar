...
# This test exercises the isolated BF16 copies, never the int8 tree.
sys.path.insert(0, str(ROOT / "ltx-2.3-serverless-bf16" / "src"))
sys.path.insert(0, str(ROOT / "ltx-2.3-serverless-bf16" / "scripts"))
...
class WorkflowApiTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        source = json.loads(
        # The legacy workflow source is a shared immutable asset, like the LoRA.
...
class Ltx25Ia2vWorkflowTests(unittest.TestCase):
    ...
    def test_default_template_uses_bf16_unet(self) -> None:
        self.assertEqual(
    ...
    def test_job_workflow_carries_bf16_checkpoint(self) -> None:
        workflow = build_job_workflow(self.template, self._values(), "a")
        self.assertEqual(workflow["130"]["inputs"]["unet_name"], UNET_NAME)
        self.assertEqual(workflow["183"]["inputs"]["filename_prefix"], "video/jobs/a/LTX_2.5_ia2v")
