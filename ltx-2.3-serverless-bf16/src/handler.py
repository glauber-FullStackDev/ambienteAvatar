...
LOG = logging.getLogger("ltx23-serverless-bf16")
...
# This image is pinned to the BF16 DiT: the default workflow template is the
# BF16 variant baked at build time.  WORKFLOW_PATH can still be overridden
# per endpoint, but nothing in this image references the int8 template.
WORKFLOW_PATH = Path(
    os.environ.get(
        "WORKFLOW_PATH",
        "/opt/defaults/workflows/video_ltx2_5_ia2v_bf16_api.json",
    )
)
